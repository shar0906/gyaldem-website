...
export default function EventsClient({
  ...
  const EventCard = ({ event, i }: { event: Event; i: number }) => (
    <div
      key={event.id}
      style={{ borderRight: i % 2 === 0 ? "0.5px solid rgba(10,10,10,0.15)" : "none", borderBottom: "0.5px solid rgba(10,10,10,0.15)", padding: "32px" }}
    >
      ...
      {event.event_type === "installation" ? (
        <span
          ...
          style={{ fontSize: "11px", letterSpacing: "0.2em", textTransform: "uppercase", color: "#0A0A0A", fontFamily: "sans-serif", textDecoration: "none", borderBottom: "1px solid #8B1A1A", paddingBottom: "2px", cursor: "pointer" }}
        >
          Learn More →
        </span>
      ) : event.rsvp_link ? (
  ...
  return (
    <main style={{ backgroundColor: "#F5F0E8", minHeight: "100vh" }}>
        ...
        <div style={{ maxWidth: "1152px", margin: "0 auto", padding: "0 24px" }}>
          ...
          {past.length > 0 && (
            <div style={{ marginBottom: "80px" }}>
              ...
              {past.map((event) => (
                <div key={event.id} style={{ display: "flex", gap: "20px", alignItems: "center", padding: "20px 0", borderTop: "0.5px solid rgba(10,10,10,0.15)" }}>
                  ...
                  {event.event_type === "installation" ? (
                    <span
                      onClick={() => setActiveInstallation(event)}
                      style={{ fontSize: "11px", letterSpacing: "0.2em", textTransform: "uppercase", color: "#0A0A0A", fontFamily: "sans-serif", borderBottom: "1px solid #8B1A1A", paddingBottom: "2px", cursor: "pointer", whiteSpace: "nowrap" }}
                    >
                      View Activation →
                    </span>
                  ) : event.rsvp_link ? (
                    <a href={event.rsvp_link} style={{ fontSize: "11px", letterSpacing: "0.2em", textTransform: "uppercase", color: "#0A0A0A", fontFamily: "sans-serif", textDecoration: "none", borderBottom: "1px solid #8B1A1A", paddingBottom: "2px", whiteSpace: "nowrap" }}>View Event →</a>
                  ) : null}
                </div>
              ))}
      ...
      {activeInstallation && (
        <div
          style={{ position: "fixed", inset: 0, backgroundColor: "rgba(0,0,0,0.92)", zIndex: 200, overflowY: "auto", display: "flex", alignItems: "center", justifyContent: "center", padding: "24px" }}
          onClick={(e) => { if (e.target === e.currentTarget) setActiveInstallation(null); }}
        >
          <div style={{ backgroundColor: "#F5F0E8", maxWidth: "680px", width: "100%", position: "relative" }}>
            ...
            <div style={{ padding: "32px" }}>
              <p style={{ color: "#8B1A1A", fontSize: "11px", letterSpacing: "0.2em", textTransform: "uppercase", fontFamily: "sans-serif", margin: "0 0 8px" }}>
                Installation · {formatDate(activeInstallation.date)}{activeInstallation.location ? ` · ${activeInstallation.location}` : ""}
              </p>
            ...
            <button
              ...
              style={{ position: "absolute", top: "16px", right: "16px", background: "rgba(0,0,0,0.5)", opacity: "50%", border: "none", color: "white", width: "36px", height: "36px", cursor: "pointer", fontSize: "18px", display: "flex", alignItems: "center", justifyContent: "center" }}
            >
              ×
            </button>
          </div>
